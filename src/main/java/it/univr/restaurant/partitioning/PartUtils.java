...
public class PartUtils {
  ...
  ( File input,
    ...
    double[] xSplits,
    double[] ySplits,
    double[] sSplits ) throws IOException {
    ...
    for( RestaurantRecord r : records ){
      ...
      while( i < timeSplits.length && t == - 1 ){
        if( r.get$date() >= timeSplits[i-1] && r.get$date() <= timeSplits[i] ){
          t = i-1;
        }
        i++;
      }
      ...
      final String key = format( partTemplate, partPrefix, t, x, y, s );
  ...
  ( File input,
    ...
    final double widthSidePartS = ((double)( boundaries.getMaxScore() - boundaries.getMinScore() )) / numCellPerSide;
    ...
    try( BufferedReader br = new BufferedReader( new FileReader( input ) ) ) {
      try( BufferedWriter wr = new BufferedWriter( new FileWriter( tempFile ) ) ) {
        ...
        while( ( line = br.readLine() ) != null ) {
          ...
          final int xPart;
          if( r.getCoordX() == boundaries.getMaxX() ){
            xPart = numCellPerSide - 1;
          } else {
            xPart = (int) ( ( r.getCoordX() - boundaries.getMinX() ) / widthSidePartX );
          }

          // the cell index is given by the integer part except for the max boundary
          final int yPart;
          if( r.getCoordY() == boundaries.getMaxY() ){
            yPart = numCellPerSide - 1;
          } else {
            yPart = (int) ( ( r.getCoordY() - boundaries.getMinY() ) / widthSidePartY );
          }

          // the cell index is given by the integer part except for the max boundary
          final int tPart;
          if( r.get$date() == boundaries.getMaxT() ){
            tPart = numCellPerSide - 1;
          } else {
            tPart = (int) ( ( r.get$date() - boundaries.getMinT() ) / widthSidePartT );
          }

          // the cell index is given by the integer part except for the max boundary
          final int sPart;

          if( r.getScore() == boundaries.getMaxScore() ){
            sPart = numCellPerSide - 1;
          } else {
            sPart = (int) ( ( r.getScore() - boundaries.getMinScore() ) / widthSidePartS );
          }

          final String key = format( partTemplate, xPart, yPart, tPart, sPart );
          keySet.add( key );
  ...
  ( File input,
    ...
    for( String k : yParts.keySet() ) {
      ...
      for( RestaurantRecord r : yParts.get( k )) {
        // for each temporal split
        if (r.getScore() != null) {
          //int sPart = (int) ((r.getAge() - minA) / widthSidePartA );
          // the cell index is given by the integer part except for the max boundary
          final int sPart;
