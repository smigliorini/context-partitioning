 ...
 * be stored. If mutable objects are stored in the pair, then the pair itself
...
public abstract class Pair<L, R> implements Map.Entry<L, R>, Comparable<Pair<L, R>>, Serializable {
   ...
   *
   * <p>This uses {@link java.util.Formattable} to perform the formatting. Two
   * variables may
   * be used to embed the left and right elements. Use {@code %1$s} for the left
   * element (key) and {@code %2$s} for the right element (value). The default
   * format used by {@code toString()} is {@code (%1$s,%2$s)}.</p>
   *
